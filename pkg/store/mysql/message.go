...
func (dao *MessageDAO) Create(
	...
	mysqlMessage := MessageStoreSchema{
		UUID:       message.UUID,
...
func (dao *MessageDAO) CreateMany(
	...
	mysqlMessages := make([]MessageStoreSchema, len(messages))
	for i, msg := range messages {
		mysqlMessages[i] = MessageStoreSchema{
			UUID:       msg.UUID,
			SessionID:  dao.sessionID,
			Role:       msg.Role,
			Content:    msg.Content,
	...
	messages = messagesFromStoreSchema(mysqlMessages)
...
func (dao *MessageDAO) GetEmbeddingListBySession(ctx context.Context) ([]models.TextData, error) {
	...
	for i, vectorStoreRecord := range results {
		embeddings[i] = models.TextData{
			Embedding: vectorStoreRecord.Embedding,
