...
func (dc *DocumentCollectionDAO) Create(
	...
	if dc.appState.Config.Store.MySQL.AvailableIndexes.HSNW {
		dc.IndexType = "hnsw"
...
func (dc *DocumentCollectionDAO) GetCollectionCounts(
	...
	err := dc.db.NewSelect().
		...
		ColumnExpr("SUM(is_embedded) as document_embedded_count").
		Scan(ctx)
	if err != nil {
...
// The tableName needs to be less than 64 characters long for MySQL, so we limit the
...
func generateDocumentTableName(collection *DocumentCollectionDAO) (string, error) {
	...
	if len(tableName) > 64 {
		return "", fmt.Errorf("table name too long: %d > 64 char maximum", len(tableName))
	}
