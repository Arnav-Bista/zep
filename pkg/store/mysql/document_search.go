...
func (dso *documentSearchOperation) Execute() (*models.DocumentSearchResultPage, error) {
	...
	err = dso.db.RunInTx(dso.ctx, &sql.TxOptions{}, func(ctx context.Context, tx bun.Tx) error {
		...
		case "ivfflat":
			// In MySQL, you would use the appropriate settings for your index type if required.
			// This is just an example and may need to be adjusted based on your MySQL setup.
			if dso.collection.IsIndexed {
				_, err = tx.Exec("SET SESSION innodb_ft_result_cache_limit = ?", dso.collection.ProbeCount)
			} else {
				_, err = tx.Exec("SET SESSION max_parallel_workers_per_gather = ?", MaxParallelWorkersPerGather)
			}
			if err != nil {
		...
		case "hnsw":
			if dso.collection.IsIndexed {
				_, err = tx.Exec("SET SESSION hnsw.ef_search = ?", DefaultEFSearch)
			} else {
				_, err = tx.Exec("SET SESSION max_parallel_workers_per_gather = ?", MaxParallelWorkersPerGather)
			}
		default:
...
func (dso *documentSearchOperation) buildQuery(db bun.IDB) (*bun.SelectQuery, error) {
	...
	if dso.searchPayload.Text != "" || len(dso.searchPayload.Embedding) != 0 {
		...
		// Score is cosine similarity normalized to 1 (example calculation for MySQL)
...
func (dso *documentSearchOperation) getDocQueryVector(
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get document embedding model %w", err)
	}
