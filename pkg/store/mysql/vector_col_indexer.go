...
// MinRowsForIndex is the minimum number of rows required to create an index. The recommendation
// is to create the index after a representative sample of data is loaded. This is a guesstimate.
...
func (vci *VectorColIndex) CreateIndex(_ context.Context, force bool) error {
	...
	go func() {
		...
		_, err = db.ExecContext(
			ctx,
			"CREATE INDEX ? ON ? (ST_Distance_Sphere(point(embedding->'x', embedding->'y'), point(?, ?)))",
			bun.Ident(indexName),
